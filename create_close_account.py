...
def extract_pool_info(pools_list: list, mint: str) -> dict:
    for pool in pools_list['official']:

        
        if pool['quoteMint'] == mint and pool['baseMint'] == 'So11111111111111111111111111111111111111112':
            return pool
        elif pool['baseMint'] == mint and pool['quoteMint'] == 'So11111111111111111111111111111111111111112':
            return pool
    for pool in pools_list['unOfficial']:
...
def fetch_pool_keys(mint: str):
    ...
    try:
        ...
        with open('all_pools.json', 'w') as file:
             json.dump(pools, file)
        try:
            amm_info = extract_pool_info(pools, mint)
        except:
            return "failed"
